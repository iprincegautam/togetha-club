#!/usr/bin/env node
/**
 * Apply migration 025 (dm_leads table) via direct Postgres connection.
...
const sql = readFileSync(
  resolve(root, 'supabase/migrations/025_create_dm_leads_table.sql'),
  'utf8'
)
...
try {
  ...
  const { rows } = await client.query(
    "SELECT to_regclass('public.dm_leads') AS table_name"
  )
