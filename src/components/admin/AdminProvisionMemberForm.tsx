...
export default function AdminProvisionMemberForm({
  packagePriceLabel,
}: AdminProvisionMemberFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/admin/members/provision', {
      method: 'POST',
    ...
    <div className="admin-card admin-panel">
      ...
      {result && (
        <div className="admin-panel" style={{ marginTop: 20, background: '#e8f5e9' }}>
          ...
          <Link href={ROUTES.adminApplicant(result.applicantId)} className="admin-btn">
            View applicant →
          </Link>
        </div>
      )}
