...
export default function AccountResetPasswordForm() {
  ...
  const resetWithOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`${ROUTES.accountLogin}?email=${encodeURIComponent(email)}&reset=1`)
    } catch (err) {
  ...
  return (
    <div className="apply-shell" style={{ maxWidth: 420 }}>
      ...
      <form className="apply-card" onSubmit={hasSession ? resetWithSession : resetWithOtp}>
        {!hasSession && (
          <>
            <div className="apply-field">
              <label className="apply-label" htmlFor="reset-email">Email</label>
              <input
                id="reset-email"
                type="email"
                required
        ...
        <div className="apply-field">
          <label className="apply-label" htmlFor="reset-password">New password</label>
          <input
            id="reset-password"
            type="password"
            required
        ...
        <div className="apply-field">
          <label className="apply-label" htmlFor="reset-confirm">Confirm password</label>
          <input
            id="reset-confirm"
            type="password"
            required
      ...
      <div className="auth-links">
        <Link href={ROUTES.accountForgotPassword}>Request a new code</Link>
